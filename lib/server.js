...
// ─── Body Parsers ─────────────────────────────────────────────────────────────
// JSON body limit: 1MB — prevents DoS via huge JSON payloads (F09-A)
...
// Disable when DISABLE_RATE_LIMIT=true (used by test suite to prevent test interference)
...
// Initialize all data (meal plan + recipes + shopping list)
...
// Update weather for all planned meals
app.post('/api/plan/week', async (req, res) => {
  try {
    ...
    // Update weather for this week's planned meals
    forecast.forEach(day => {
