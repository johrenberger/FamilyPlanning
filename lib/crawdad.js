...
function loadConfig() {
  ...
  return {
    ...
    stores: [
      { name: 'Kroger', aisles: ['Produce', 'Deli', 'Dairy', 'Meat', 'Bakery', 'Frozen', 'Pantry', 'Beverages', 'Snacks', 'Cleaning'] },
      { name: 'Costco', aisles: ['Produce', 'Deli', 'Dairy', 'Meat', 'Bakery', 'Frozen', 'Pantry', 'Electronics', 'Home'] },
      { name: "Trader Joe's", aisles: ['Produce', 'Deli', 'Dairy', 'Wine', 'Frozen', 'Pantry', 'Snacks'] }
    ],
    cooks: ['You', 'Wife'],
    rotationWeeks: 2,
        ...
        res.on('end', () => {
          try {
            ...
            resolve({
              ...
              rain: current.precipMM > 0,
              chanceOfRain: parseInt(current.precipInMm[0]) > 30
            });
          } catch (e) {
        ...
        res.on('end', () => {
          try {
            ...
            const forecast = json.weather.slice(0, days).map(day => ({
              ...
              high: day.maxTempF + '°F',
              low: day.minTempF + '°F',
              condition: day.hourlySummary || day.description,
              goodForGrilling: day.manch?.[0]?.value !== 'Rain' && parseInt(day.maxTempF) > 60 && parseInt(day.maxTempF) < 95,
...
function getStoreForIngredient(ingredient, stores) {
  ...
  if (organic.some(i => ing.includes(i))) return stores[2]?.name || "Trader Joe's";
  return stores[0]?.name || 'Kroger';
}
